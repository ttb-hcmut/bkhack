...
We need to inform the build system that for a Reason page file $ x $ there is a mapping to a JavaScript bundle $ y $ at a url $ y' $. In early versions, I (kinten) tested using a dictionary placed in the script file to define this mapping. In newer versions, I have decided to utilized Reason's infile floating attributes [^attributes]. This is a native way to provide decorator-like metadata to a file / module unit.
