...
let () =
  ...
  exit @@ Cmd.eval @@
	Cmd.v(Cmd.info("typst", ~doc="")) @@ {
	let open Term.Syntax;
  let+ outdir = Arg.(required & opt((some(string)), None) &
    info(["output", "o"], ~doc=" Output directory. ")) |> Term.map(Path.((it, cwd) => cwd / it))
  and+ package_path = Arg.(required & opt((some(string)), None) &
    info(["package-path"], ~doc=" Package directory. ")) |> Term.map(Path.((it, cwd) => cwd / it))
	and+ article_at_root = Arg.(required & opt((some(bool)), None) & info(["article-at-root"]))
	and+ `typst_compile(bin, args) = Arg.(value & pos_all(string, []) &
		info([], ~docv="haha")) |> Term.map([@warning "-8"] fun | ["typst", "compile", ...args] => `typst_compile(["typst", "compile"], args) | [bin, ...args] => `typst_compile([bin], args));
	Eio_main.run @@ env => {
