...
We assume that the users are new-comer computer-science interests, we
want to teach them about stream-based programming and its aspects.\
...
where ```sh split``` has the option #box(```sh -c NUM```) where `NUM` is the
count size of each chunk. Conventionally, the POSIX ```sh split``` command
accepts the option #box(```sh -l NUM```) where `NUM` is line number size of chunk
.\
...
The grammar of the #o.bkhack-shell language, given in EBNF form in @gr.
...
_option parser_, such as POSIX `getopts`. Parsing is implemented based on the grammar at @gr.
#place(auto, float: true, scope: "parent")[
  #figure(caption: [Grammar for the #o.bkhack-shell language], shell-parse.langchain) <gr>
]
// #lorem(30) For example, one possible parse of the command at the preamble is
// #align(center)[
...
// ]
// where #lorem(20)
  In the initial design, we have only one phase for our parser. This is
an eager, AST-less, final a.k.a. syntax-directed parser--effects are
eagerly evaluated as each term is parsed. Eventually, we did make it so
that it outputs an AST, for reusability and convenience.\
  In the next design, we have two phases for our parser: the lexical
analysis phase (bộ phân tích từ vựng @giao-trinh-trinh-bien-dich-lexer)
#fn[a.k.a. character grouping] and the parsing phase. The lexical analysis
phase is to guarantee a minimal schema for the parsers so that they won't
diverge from the language spec. This is an _implicit structure_. In contrast
, we could have defined an ADT or GADT to make an explicit structure. However
, our parsers won't be final, we would make a compromise on performance.
Not only that, an implicit structure allows room for undefined behaviors
...
The combinators are designed based on the typing rules at @typ. An example
usage of these combinators would be
