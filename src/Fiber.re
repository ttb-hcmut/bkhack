...
let rec of_worker =
	...
	{ let tbl = Hashtbl.create(4) and idgen = ref(5)
		let worker = mkworker() |> onmessage(~tbl) |> onerror(~tbl);
...
and submit = (worker, ~idgen, ~tbl, in_) => {
	...
	{ tbl->Hashtbl.add(id, (resolve, reject));
		Js__worker.Worker.postMessage(worker, (id, in_)) }
}

and onmessage = (worker, ~tbl) => { worker->Js__worker.Worker.onmessage(e => {
	open { [@mel.send] external call1 : Js.Fn.arity1('a => 'b) => 'this => 'a => 'b = "call"; };
	let (owner, res) = Js__worker.Message.data(e);
	let (resolve, reject as reject_) = tbl->Hashtbl.find(owner);
	switch (res) {
...
and onerror = (worker, ~tbl) => { worker->Js__worker.Worker.onerror(e => {
