...
module ItemNav = {
  ...
  [@react.component]
  let make = (~id, ~currentTab, ~setCurrentTab, ~prCount) => {
	...
	let make = () => {
		...
		<>
			...
			<nav className=sidebarState>
				<ItemNav id={Option.get(Util.parseQueryParams(url.search) -> Js.Dict.get("id"))} currentTab=tab setCurrentTab prCount=Array.length(pullrequests)/>
			</nav>
