...
module Timer = {
  ...
  let make = () => {
    ...
    // let (count,setCount) = React.useState(()=>0);
		...
		let n = React.useMemo0(() => Fiber.With_ctrl.make(~ctrl, p'));
		let m = React.useMemo0(() => Fiber.With_ctrl.make(~ctrl, q'));
    // let balls = ;
    let run' = k => {
      ignore(Fetch__syntax.({
				let* u = Fiber.With_ctrl.run_promise(~ctrl, (interval), k);
        // u ;
				if (false) { ignore(u+1) };
				Js.Console.log(u);
				return(())
    ...
    <div> 
      ...
      <button onClick={_=>run()}>{React.string("StartTimer")}</button>
